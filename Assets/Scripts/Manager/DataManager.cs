...
public class DataManager
{
    ...
    #region === QUEST SAVE SYSTEM ===

    public static void UpdateQuestProgressByIndex(int questIndex, int currentKills)
    {
        CurrentData.activeQuestIndex = questIndex;
        CurrentData.currentQuestProgress = currentKills;
    }

    public static void CompleteQuestByIndex(int questIndex)
    {
        if (!CurrentData.completedQuests.Contains(questIndex.ToString()))
            CurrentData.completedQuests.Add(questIndex.ToString());

        if (CurrentData.activeQuestIndex == questIndex)
        {
            CurrentData.activeQuestIndex = -1;
            CurrentData.currentQuestProgress = 0;
        }
    }

    public static void UpdateKeyQuestProgress(int collected, int required, bool completed)
    {
        CurrentData.crystalsCollected = collected;
        CurrentData.keyQuestCompleted = completed;
    }
