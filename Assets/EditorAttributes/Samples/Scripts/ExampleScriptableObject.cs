...
namespace EditorAttributesSamples
{
    ...
    public class ExampleScriptableObject : ScriptableObject
    {
        ...
        [Title("General Settings")] public string gameName;
        public bool isMultiplayer;

        [ShowField(nameof(isMultiplayer)), Clamp(0, 5), IndentProperty(20f)]
        public int maxPlayers;

        [Space] [Title("Entity Settings", titleSpace: 20f)] [SerializeField]
        private Void titleHolder;

        [HelpBox("Will apply randomly to each player in the list", MessageMode.None)] [MinMaxSlider(0, 100)]
        public Vector2Int startingCurrency;

        [DataTable] public PlayerClass[] playerClasses;
        [Space] [DataTable(true)] public EnemyData[] enemies;
        ...
        [Space] [Dropdown(nameof(GetAudioClips))]
