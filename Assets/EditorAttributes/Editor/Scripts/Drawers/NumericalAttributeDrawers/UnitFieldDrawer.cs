...
namespace EditorAttributes.Editor
{
    ...
    public class UnitFieldDrawer : PropertyDrawerBase
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            var convertedUnit =
                UnitConverter.GetConversion(unitFieldAttribute.DisplayUnit, unitFieldAttribute.ConversionUnit);
            ...
            void OnGeometryChanged(GeometryChangedEvent changeEvent)
            {
                ...
                root.Query<BindableElement>(className: TextInputBaseField<Void>.ussClassName).ForEach((element) =>
                {
                    ...
                    ExecuteLater(root, () =>
                    {
                        ...
                        var textCopy = fieldCopy.Q(className: TextInputBaseField<Void>.inputUssClassName)
                            .Q<TextElement>();
        ...
        private VisualElement CreateFieldCopy(BindableElement element, SerializedProperty property,
            UnitFieldAttribute unitFieldAttribute)
        {
            var convertedUnit =
                UnitConverter.GetConversion(unitFieldAttribute.DisplayUnit, unitFieldAttribute.ConversionUnit);
            ...
            var elementCopy = CreateFieldForType(propertyType, GetFieldLabel(element),
                Convert.ChangeType(Convert.ToDouble(GetFieldValue(element)) / convertedUnit.conversion, propertyType),
                property.hasMultipleDifferentValues);

            RegisterValueChangedCallbackByType(propertyType, elementCopy,
                (changeEvent) => SetFieldValue(element,
                    Convert.ChangeType(Convert.ToDouble(GetFieldValue(elementCopy)) * convertedUnit.conversion,
                        propertyType), true));

            element.TrackPropertyValue(property,
                (trackedProperty) => SetFieldValue(elementCopy,
                    Convert.ChangeType(Convert.ToDouble(GetFieldValue(element)) / convertedUnit.conversion,
                        propertyType)));
