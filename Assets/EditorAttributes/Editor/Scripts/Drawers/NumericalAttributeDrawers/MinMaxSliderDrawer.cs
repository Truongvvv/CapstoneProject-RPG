    ...
    public class MinMaxSliderDrawer : PropertyDrawerBase
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
                ...
                property.propertyType == SerializedPropertyType.Vector2Int)
            {
                ...
                root.TrackPropertyValue(property,
                    (trackedProperty) => minMaxSlider.value =
                        isIntVector ? trackedProperty.vector2IntValue : trackedProperty.vector2Value);
                ...
                if (minMaxSliderAttribute.ShowValues)
                {
                    ...
                    {
                        minField.SetValueWithoutNotify(isIntVector
                            ? Mathf.RoundToInt(callback.newValue.x)
                            : callback.newValue.x);
                        maxField.SetValueWithoutNotify(isIntVector
                            ? Mathf.RoundToInt(callback.newValue.y)
                            : callback.newValue.y);
                    ...
                    minField.RegisterValueChangedCallback((callback) =>
                    {
                        ...
                        minMaxSlider.value = isIntVector
                            ? new Vector2Int(Mathf.RoundToInt(callback.newValue),
                    ...
                    maxField.RegisterValueChangedCallback((callback) =>
                    {
                        ...
                        minMaxSlider.value = isIntVector
                            ? new Vector2Int(Mathf.RoundToInt(minMaxSlider.value.x),
        ...
        protected override void PasteValue(VisualElement element, SerializedProperty property, string clipboardValue)
        {
            ...
            minMaxSlider.value = property.propertyType == SerializedPropertyType.Vector2
