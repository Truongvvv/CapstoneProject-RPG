...
namespace EditorAttributes.Editor
{
    ...
    public class ButtonFieldDrawer : PropertyDrawerBase
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            if (function == null)
                return new HelpBox(
                    $"Could not find function <b>{buttonFieldAttribute.FunctionName}</b>. If this function is inherited make sure is marked at protected.",
            ...
            if (functionParameters.Length == 0)
            {
                if (buttonFieldAttribute.IsRepetable)
                {
                    var repeatButton =
                        new RepeatButton(
                            () => InvokeFunctionOnAllTargets(property.serializedObject.targetObjects, function.Name),
