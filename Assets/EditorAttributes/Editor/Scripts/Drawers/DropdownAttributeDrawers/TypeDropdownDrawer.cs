    ...
    public class TypeDropdownDrawer : PropertyDrawerBase
    {
        public override VisualElement CreatePropertyGUI(SerializedProperty property)
        {
            ...
            if (property.propertyType == SerializedPropertyType.String)
            {
                ...
                var typeDropdown = new DropdownField(property.displayName, dropdownValues,
                    GetDropdownDefaultValue(dropdownValues, property))
                {
                ...
                typeDropdown.TrackPropertyValue(property, (trackedProperty) =>
                {
                    ...
                    {
                        Debug.LogWarning(
                            $"The value <b>{trackedProperty.stringValue}</b> set to the <b>{trackedProperty.name}</b> variable is not a valid type string.",
                ...
                ExecuteLater(typeDropdown,
                    () => typeDropdown.Q(className: DropdownField.inputUssClassName).style.backgroundColor =
                        EditorExtension.GLOBAL_COLOR / 2f);
            }
            else
            {
                root.Add(new HelpBox("The TypeDropdown attribute can only be attached to string fields",
        ...
        protected override void PasteValue(VisualElement element, SerializedProperty property, string clipboardValue)
        {
            ...
            {
                Debug.LogWarning(
                    $"Could not paste value \"{dropdownValue}\" since is not availiable as an option in the dropdown");
            }
        }
        ...
        private List<string> GetAvailableTypes()
        {
            ...
            var typeCollection = typeDropdownAttribute.AssemblyName == string.Empty
                ? TypeCache.GetTypesDerivedFrom<object>()
                : TypeCache.GetTypesDerivedFrom<object>(typeDropdownAttribute.AssemblyName);
