...
namespace EditorAttributes.Editor
{
    public class PropertyDrawerBase : PropertyDrawer
    {
        ...
        protected void SetPropertyValueFromString(string value, SerializedProperty property)
        {
            ...
            {
                Debug.LogError($"Could not convert the value \"{value}\" to <b>{property.propertyType}</b>",
                    property.serializedObject.targetObject);
            }
        }
        ...
        /// <returns>The scheduled visual element item</returns>
        public static IVisualElementScheduledItem ExecuteLater(VisualElement visualElement, Action logicToExecute,
            long delayMs = 1) => visualElement.schedule.Execute(logicToExecute).StartingIn(delayMs);
        ...
        public static VisualElement CreateFieldForType<T>(string fieldName, object fieldValue,
            bool showMixedValue = false) => CreateFieldForType(typeof(T), fieldName, fieldValue, showMixedValue);
            ...
            object targetObject)
        {
            UpdateVisualElement(field, () =>
            {
                ...
                if (IsTypeValid(fieldType))
                {
                    SetFieldValue(field, memberValue);
                         ...
                         !ReflectionUtility.IsTypeCollection(fieldType))
                {
            ...
            static bool IsTypeValid(Type type) => type.IsEnum
                                                  ...
                                                  || type == typeof(LayerMask) || type == typeof(Rect) ||
                                                  type == typeof(RectInt) || type == typeof(Bounds) ||
                                                  type == typeof(BoundsInt) || type == typeof(Gradient) ||
